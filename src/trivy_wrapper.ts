...
export class TrivyWrapper {
   ...
   async scanArtifact(scanType: string) {
        let command: string[] = [];
        ...
        if (scanType === 'docker') {
            const dockerImage = await vscode.window.showInputBox({
                prompt: 'Enter Docker image name (e.g., "ubuntu:latest")',
                placeHolder: 'e.g., "my-image:tag"'
            });
            ...
            const sbomFilePath = await vscode.window.showInputBox({
                prompt: 'Enter the path for the SBOM file',
                placeHolder: 'e.g., "/path/to/sbom.json"'
            });
    ...
    run() {
        ...
        this.workingPath.forEach(workingPath => {
            let command = this.buildCommand(workingPath, 'fs');
            this.outputChannel.appendLine(`command: ${command}`);
    ...
    private buildCommand(workingPath: string, scanType: string): string[] {
        const config = vscode.workspace.getConfiguration('trivy');
        var command = [];
